...
install_cpprestsdk(){
   ...
   if [ -d "$restsdkDir" ]; then
	...
	git clone https://github.com/conan-repos/cpprestsdk.git "$restsdkDir"
	(cd $restsdkDir && git checkout tags/$RESTSDK_VERSION -b $RESTSDK_VERSION && git submodule update --init)
